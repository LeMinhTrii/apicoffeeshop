    ...
    get: (req, res) => {
        db.query(sql.getAllUser, (err, response) => {
            if (Object.entries(response).length === 0 || err) {
                res.status(404).json({
                    message: "Not Found - Tài nguyên bạn muốn truy xuất không tồn tại hoặc đã bị xóa.",
                });
            }
            ...
            if (response.length === 0) {
                ...
                db.query(sql.updateUserById, [user, id], (err, response) => {
                    if (err) {
                        ...
                        res.status(422).json({
                            message: "Unprocessable Entity - Dữ liệu của bạn gửi lên không hợp lệ hoặc bị lỗi.",
                        });
                    }
    ...
    updateCoverImage: (req, res) => {
        ...
        db.query(sql.getCoverImageById, id, (err, response) => {
            if (response.length === 0) {
                ...
                db.query(
                    sql.updateCoverImageById, [coverimage, id],
                    (err, response) => {
                        if (err) {
                            if (fileData) cloudinary.uploader.destroy(fileData.filename);
                            res.status(422).json({
